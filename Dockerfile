...
# OpenSSL
    && openssl version \
    && curl -fSL https://www.openssl.org/source/openssl-1.1.1f.tar.gz -o ${BUILD_SRC}/openssl-1.1.1f.tar.gz \
    && tar xzf ${BUILD_SRC}/openssl-1.1.1f.tar.gz -C ${BUILD_SRC} \
    && cd ${BUILD_SRC}/openssl-1.1.1f \
    && ./config \
    && make \
    && make install \
    && openssl version \
